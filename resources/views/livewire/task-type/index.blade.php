    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        ...
        <div class="mt-8 mb-6 p-1 flex justify-between items-center">
            <div class="text-sm text-gray-500">
                Showing {{ $taskType->firstItem() }} to {{ $taskType->lastItem() }} of {{ $taskType->total() }} entries
            </div>
            <div class="flex items-center space-x-1">

                {{-- Previous Button --}}
                @if ($taskType->onFirstPage())
                ...
                {{-- Pagination Numbers --}}
                @foreach ($taskType->getUrlRange(1, $taskType->lastPage()) as $page => $url)
                    @if ($page == $taskType->currentPage())
