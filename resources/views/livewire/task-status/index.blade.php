<div>
    <div class="breadcrumbs text-sm mt-4">
        <ul>
            ...
            <li><a href="{{ route('task-status.index') }}" class="text-gray-400 font-semibold">Task Status</a></li>
        </ul>
    </div>
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        ...
        <div class="mt-8 mb-6 p-1 flex justify-between items-center">
            <div class="text-sm text-gray-500">
                Showing {{ $taskStatus->firstItem() }} to {{ $taskStatus->lastItem() }} of {{ $taskStatus->total() }} entries
            </div>
            <div class="flex items-center space-x-1">

                {{-- Previous Button --}}
                @if ($taskStatus->onFirstPage())
                ...
                {{-- Pagination Numbers --}}
                @foreach ($taskStatus->getUrlRange(1, $taskStatus->lastPage()) as $page => $url)
                    @if ($page == $taskStatus->currentPage())
